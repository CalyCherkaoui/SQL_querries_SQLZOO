...
-- COALESCE , INNER JOIN, LEFT JOIN, RIGHT JOIN, IS NULL
...
You might think that the phrase dept=NULL would work here but it doesn't
 - you can use the phrase dept IS NULL*/
...
SELECT (CASE WHEN teacher.dept=1 OR teacher.dept=2
                  THEN CONCAT('Sci ',teacher.name)
             ELSE 
                  CONCAT('Art ',teacher.name)
        END) name
  FROM teacher;
...
SELECT name, (CASE WHEN teacher.dept=1 OR teacher.dept=2
                        THEN 'Sci'
                   WHEN teacher.dept=3
                        THEN 'Art '
                   ELSE 'None'
